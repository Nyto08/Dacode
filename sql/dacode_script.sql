...
-- drop table if exists WkLessModel;
...
-- CREATE TABLE WkLessModel (
--     id_wklesmod INT AUTO_INCREMENT,
--     id_wk INT,
--     id_les INT,
--     PRIMARY KEY (id_wklesmod),
--     FOREIGN KEY (id_wk) REFERENCES Workspace(id_wk) ON UPDATE CASCADE ON DELETE CASCADE,
--     FOREIGN KEY (id_les) REFERENCES Lesson(id_les) ON UPDATE CASCADE ON DELETE CASCADE
-- ) ENGINE=InnoDB;
...
create index idx_wk_les_fork ON WkLessFork(id_wklesfork);
create index idx_wk_les_modl ON WkLessModel(id_wklesmod);
...
CREATE FUNCTION create_model_less_workspace (idUser INT, idLes INT) RETURNS INT
BEGIN
    INSERT INTO Workspace (id_user) VALUES (idUser);

    SET @idWk = LAST_INSERT_ID();

    INSERT INTO WkLessModel (id_wk, id_les) VALUES (@idWk, idLes);
...
BEGIN
    ...
    SELECT id_cod FROM Code WHERE id_wk IN (SELECT id_wk FROM WkLessModel WHERE WkLessModel.id_les = idLes)
        INTO @id_cod;
